...
// import ArticleFeature from '../components/blog/ArticleFeature';
...
">
        {/* Search */}
        <SearchFilter  className="-mt-16 md:-mt-24"
          q={q}
          setQ={setQ}
          count={effectiveCount}
          category={category}
        ...
        {/* ✅ Category filter (ONLY CHANGE HERE) */}
        <section className="mt-12">
          <Category onChange={setCategory} />
        </section>
        ...
        <section className="mt-6">
          {loading ? (
            ...
            <p className=" flex  justify-center items-center text-white/70 font-bold font[`Halvetica neun` ]">No blogs found...</p>
          ) : (
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {posts.map((post) => (
                <Card key={post.id} post={post} />
